"""
Train LightGBM classifier on MERT embeddings.
Usage:
  python3 train_mert_model.py --data-dir ~/Desktop/classified_music_dataset --out-dir artifacts
"""
...
def train(features_path, out_dir):
    ...
    feature_cols = [c for c in df.columns if c.startswith("mert_")]
    ...
    with open(os.path.join(out_dir, "model_type.json"), 'w') as f:
        json.dump({"type": "mert", "model_id": "m-a-p/MERT-v1-95M"}, f)

    print(f"[INFO] Saved model -> {out_dir}/genre_model.joblib")
    print(f"[INFO] Model type: MERT embeddings (768-dim)")

if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--data-dir", default=None)
    ap.add_argument("--features", default="artifacts/mert_features.parquet")
    ...
    if args.data_dir:
        import torch
        from extract_mert_features import extract_dataset
        device = "mps" if torch.backends.mps.is_available() else "cpu"
        extract_dataset(args.data_dir, args.features, device, args.max_secs)
